...
class HotelManager:
    ...
    def guest_checkout(self, room_key):
        # CHANGE EXCEPTIONS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        """This third function receives a room key (of the type generated by function 2, SHA256 hexadecimal string), checks that it is valid and also
        checks that the checkout is being done the same day as scheduled. If both things are correct, it returns True and saves into a file the
        timestamp of the departure and the room key value."""
        # We check if the key is valid. To do so, we compare the key provided to the keys generated by each of our reservations.
        try:
            # Open the JSON file and load its contents.
            with open('../Reservations.json', 'r', encoding='utf-8') as f:
        ...
        for item in arrivalData:
            ...
            if not localizer_found:
                # No data in arrivals.json
                raise hotelManagementException("No arrivals saved.")
        ...
        if not valid_key:
            raise hotelManagementException("Key is not correct.")
        ...
        if time1 != time2:
            raise hotelManagementException("Client is not supposed to departure today.")
