...
class HotelManager:
    ...
    def guest_checkout(self, room_key):
        ...
        # reservations.
        # First we open the JSON files "Arrival.json" and "Reservations.json" and load their contents.
        try:
            ...
            raise hotelManagementException("Arrivals file not found or empty file.")
        try:
            with open('../Reservations.json', 'r', encoding='utf-8') as f:
                reservationsData = json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
            # If the file does not exist or it is empty, we raise an error.
            raise hotelManagementException("Reservations file not found or empty file.")

        # We iterate through our reservations until we find a key that matches the one provided.
        ...
        # date established in Reservations.json (arrival_date + num_days) and compare it to the current date (only taking the year,
        ...
        json_string = {"departure": str(time_now),
                     "room_key": room_key}
        try:
            with open('../checkOut.json', 'r') as f:
                checkout_data = json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
            # If the file does not exist or is empty, we initialize checkout_data as an empty list.
            checkout_data = []
        checkout_data.append(json_string)
        # We write updated data back to file.
